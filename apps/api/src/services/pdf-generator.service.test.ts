/**
 * Tests for PDF Document Generator Service.
 *
 * Tests the PDF document generation functionality including:
...
import { generatePdfDocument } from './pdf-generator.service.js';
import type { PdfGeneratorInput } from './pdf-generator.service.js';
import type {
  ReportNode,
  ReportRiskSummary,
...
describe('PDF Generator Service', () => {
  ...
  const createDefaultInput = (overrides?: Partial<PdfGeneratorInput>): PdfGeneratorInput => ({
  ...
  describe('generatePdfDocument', () => {
    it('should generate a document with correct MIME type', async () => {
      ...
      const result = await generatePdfDocument(input);
    ...
    it('should generate correct filename format', async () => {
      ...
      expect(result.filename).toMatch(/^HazOps_Report_My_Test_Analysis_\d{4}-\d{2}-\d{2}\.pdf$/);
    });
    ...
    it('should truncate long analysis names in filename', async () => {
      ...
      const namePart = result.filename.replace('HazOps_Report_', '').replace(/_\d{4}-\d{2}-\d{2}\.pdf$/, '');
