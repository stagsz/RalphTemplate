/**
 * Tests for Word Document Generator Service.
 *
 * Tests the Word document generation functionality including:
...
import { generateWordDocument } from './word-generator.service.js';
import type {
  WordGeneratorInput,
  ReportNode,
  ReportRiskSummary,
...
describe('Word Generator Service', () => {
  ...
  const createDefaultInput = (overrides?: Partial<WordGeneratorInput>): WordGeneratorInput => ({
  ...
  describe('generateWordDocument', () => {
    it('should generate a document with correct MIME type', async () => {
      const input = createDefaultInput();
      const result = await generateWordDocument(input);
    ...
    it('should generate a document with buffer', async () => {
      const input = createDefaultInput();
      const result = await generateWordDocument(input);
    ...
    it('should generate correct filename format', async () => {
      ...
      expect(result.filename).toMatch(/^HazOps_Report_My_Test_Analysis_\d{4}-\d{2}-\d{2}\.docx$/);
    });
    ...
    it('should truncate long analysis names in filename', async () => {
      ...
      const namePart = result.filename.replace('HazOps_Report_', '').replace(/_\d{4}-\d{2}-\d{2}\.docx$/, '');
    ...
    it('should handle entries with empty arrays', async () => {
      ...
      const input = createDefaultInput({ entries: [entry] });
      const result = await generateWordDocument(input);
    ...
    it('should handle analysis with null dates', async () => {
      const input = createDefaultInput({
        analysis: createMockAnalysis({
          submittedAt: null,
          approvedAt: null,
