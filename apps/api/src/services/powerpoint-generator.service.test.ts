/**
 * Tests for PowerPoint Presentation Generator Service.
 *
 * Tests the PowerPoint presentation generation functionality including:
 * - Presentation structure and slides
 * - Title, executive summary, analysis entries, risk matrix slides
 * - Compliance and recommendations slides
 * - Risk level color mapping
 * - Content filtering by risk level and node
 * - Proper MIME type and filename generation
 */

import { describe, it, expect } from '@jest/globals';
import { generatePowerPointPresentation } from './powerpoint-generator.service.js';
import type { PowerPointGeneratorInput } from './powerpoint-generator.service.js';
import type {
  ReportNode,
  ReportRiskSummary,
...
describe('PowerPoint Generator Service', () => {
  // Test data factories
  ...
  const createDefaultInput = (overrides?: Partial<PowerPointGeneratorInput>): PowerPointGeneratorInput => ({
  ...
  describe('generatePowerPointPresentation', () => {
    it('should generate a presentation with correct MIME type', async () => {
      const input = createDefaultInput();
      const result = await generatePowerPointPresentation(input);
    ...
    it('should generate a presentation with buffer', async () => {
      const input = createDefaultInput();
      const result = await generatePowerPointPresentation(input);
    ...
    it('should generate correct filename format', async () => {
      ...
      expect(result.filename).toMatch(/^HazOps_Report_My_Test_Analysis_\d{4}-\d{2}-\d{2}\.pptx$/);
    });
    ...
    it('should truncate long analysis names in filename', async () => {
      ...
      const namePart = result.filename.replace('HazOps_Report_', '').replace(/_\d{4}-\d{2}-\d{2}\.pptx$/, '');
      expect(namePart.length).toBeLessThanOrEqual(50);
    });

    it('should use custom title when provided', async () => {
      const input = createDefaultInput({
        parameters: { customTitle: 'My Custom Report Title' },
      });
      const result = await generatePowerPointPresentation(input);
